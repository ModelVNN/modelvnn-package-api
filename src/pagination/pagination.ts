...
export const pagination = async (options: PaginationOptions) => {
    ...
    const changeFooter = () => {
        ...
        if (embed?.footer?.text) {
            return newEmbed.setFooter(
                `${embed.footer.text} - Trang ${currentPage} của ${embeds.length}`,
                embed.footer.iconURL
            );
        }
        return newEmbed.setFooter(`Trang ${currentPage} của ${embeds.length}`);
    };
    ...
    const numberTravel = async () => {
        if (pageTravelling.has(author.id))
            return channel.send("Gõ `end` để dừng việc di chuyển trang!");
        const collector = channel.createMessageCollector({
            filter: (msg) => msg.author.id === author.id,
            time: 30000,
        });
        const numberTravelMessage = await channel.send(
            `${author.tag}, bạn có 30 giây, gửi số trong trò chuyện để thay đổi trang! Chỉ cần gõ \`end\` để thoát khỏi trang đang di chuyển.`
        );
